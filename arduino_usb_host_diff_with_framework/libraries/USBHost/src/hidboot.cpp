...
#define HIDDATABUF_SIZE	16
uint32_t HidDataNum = 0;
uint8_t HidDataBuf[HIDDATABUF_SIZE];

extern "C" {
void HidDataCopy(uint32_t len, uint8_t *buf) {
	if (!HidDataNum)
	{
		if (len > HIDDATABUF_SIZE)
			len = HIDDATABUF_SIZE;
		HidDataNum = len;
		while(--len){
			HidDataBuf[len] = buf[len];
		}
		HidDataBuf[0] = buf[0];
	}
}	
