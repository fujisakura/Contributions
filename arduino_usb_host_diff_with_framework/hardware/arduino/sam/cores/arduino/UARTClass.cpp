...
void UARTClass::begin( const uint32_t dwBaudRate )
{
  ...
  UART0->SMR = (SMR_MD_UART | SMR_SOE);
  UART0->SCR = 0x00;

  UART0->BGR = (SystemCoreClock / 2 + (dwBaudRate / 2)) / dwBaudRate - 1;
  UART0->ESCR = 0x0000;

  NVIC_EnableIRQ(MFS0RX_IRQn);	// Enable receive interupt
  UART0->SCR |= (SCR_RXE | SCR_TXE | SCR_RIE);
}
...
#endif
  if(UART0->SSR & SSR_RDRF)
    _rx_buffer->store_char(UART0->RDR);
}
